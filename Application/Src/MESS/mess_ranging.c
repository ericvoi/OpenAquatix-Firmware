...
void Ranging_Request(const DspConfig_t* cfg, bool feedback)
{
  ...
  Message_t request_msg = {
    ...
    .type = feedback ? MSG_TRANSMIT_TRANSDUCER : MSG_TRANSMIT_FEEDBACK,
    .delay = false
  };
...
void Ranging_LogResponse(const Message_t* msg)
{
  ...
  uint64_t one_way_cyccnt = (cyccnt_total - fixed_delay_cyccnt) / 2;
...
static bool isValidRequest(const DspConfig_t* cfg)
{
  ...
  uint16_t num_bits = PACKET_PREAMBLE_LENGTH_BITS;
  num_bits += Sync_NumSteps(cfg);

  uint32_t bit_duration_us = (uint32_t) (1.0E6 / (cfg->baud_rate));
  uint32_t packet_duration_us = num_bits * bit_duration_us;
